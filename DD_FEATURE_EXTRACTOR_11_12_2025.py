...
#        if not self.url:
#            return 0
#        return sum(self.url.count(c) for c in ["?", "%", "&", "=", "+", "￠"]) # Added Hangul Half-Width Filler or "￠" — U+FFA0 as suspicious character used in Unicode Phishing attacks

    def suspicious_chars(self) -> int:
        if not self.url:
            return 0
        # Common suspicious symbols
        chars = ["?", "%", "&", "=", "+"]
        # Unicode fillers / invisible or deceptive characters
        chars += [
            "\uFFA0",  # Hangul Half-Width Filler  "￠"
            "\u3164"   # Hangul Full-Width Filler  "  " this unicode character looks like a space but is not
        ]
        return sum(self.url.count(c) for c in chars)
    
#       https://blackswan-cybersecurity.com/wp-content/uploads/2025/04/Threat-Advisory_040425.pdf   April 4,2025
