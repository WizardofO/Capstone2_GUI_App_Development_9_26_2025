    ...
    def open_feature_extractor(self):
        try:
            subprocess.Popen([sys.executable, 
                            os.path.join(os.path.dirname(__file__), 'DD_FEATURE_EXTRACTOR_09_21_2025_B.py')],
...
# ------------------------------------------------------------------------------------------------------------------------------------------ #
    def _build_ui(self):                                                    # MMDC LOGO, TITLE, VERSION, MY LOGO @ TOP LEFT
        ...
        # Create main horizontal layout for side panel and content
        ...
        side_layout.addWidget(btn_feature_extractor)
        side_layout.addWidget(btn_model_training)
        side_layout.addStretch()
        
        # Main content layout
        main_layout = QVBoxLayout()
        
        # Add the side panel and main content to root layout
...
# ------------------------------------------------------------------------------------------------------------------------------------------ #
    def _make_phishing_page(self):
        ...
        threshold_title = QLabel("Phishing Detection Threshold")
        threshold_title.setFont(QFont("Arial", 10, QFont.Bold))
        threshold_title.setStyleSheet("color: #00796b; margin-top: 10px; margin-bottom: 5px;")
        sidebar.addWidget(threshold_title, alignment=Qt.AlignHCenter)

        slider = QSlider(Qt.Horizontal)
        slider.setMinimum(30)   # corresponds to 0.30
        slider.setMaximum(90)   # corresponds to 0.90
        slider.setSingleStep(1)
        slider.setTickInterval(5)
        slider.setValue(int(self.custom_phishing_threshold * 100))
        slider.setToolTip("Adjust phishing sensitivity (lower = more sensitive, higher = more cautious)")

        # single slot to update both internal state and label
        self.threshold_label = QLabel(f"Current threshold: {self.custom_phishing_threshold:.2f}")
        self.threshold_label.setFont(QFont("Arial", 9))
        self.threshold_label.setStyleSheet("color: #004d40; margin-bottom: 10px;")

        def _on_threshold_changed(val):
            self.custom_phishing_threshold = val / 100.0
            self.threshold_label.setText(f"Current threshold: {self.custom_phishing_threshold:.2f}")

        slider.valueChanged.connect(_on_threshold_changed)
        sidebar.addWidget(slider, alignment=Qt.AlignHCenter)
        sidebar.addWidget(self.threshold_label, alignment=Qt.AlignHCenter)
...
# ------------------------------------------------------------------------------------------------------------------------------------------ #
    def _phish_load_model(self):
        ...
        try:
            ...
            print("- Model type:", type(self.loaded_model).__name__)
            print("- Number of model features:", len(self.model_features) if self.model_features else 0)
...
# ------------------------------------------------------------------------------------------------------------------------------------------ #
    def _on_single_result(self, out: dict):
        # Feature explanations aligned with app.py - complete set
        ...
        # Get class weights from model if available
        ...
        # -------------------- Feature Importance --------------------
        if explain:
            # Format feature importance explanations
            tex = []
            for e in explain:
